...
public class PackageUtil {
     ...
     * @throws UnsupportedEncodingException
     */
//    public static List<String> getClassName0(String packageName, boolean childPackage) throws IOException {
...
//    }
    ...
    public static List<String> getClassName(String packageName, boolean childPackage) throws IOException {
        ...
        while (urls.hasMoreElements()) {
            ...
            String type = url.getProtocol();
            if (type.equals("file")) {
                fileNames.addAll(getClassNameByFile(url.getPath(), childPackage, packagePath.replace("/", "\\")));
            } else if (type.equals("jar")) {
                //fileNames.addAll(getClassNameByJar(url.getPath(), childPackage));
            }
        }
    ...
    private static List<String> getClassNameByFile(String filePath, boolean childPackage, String packagePath) throws UnsupportedEncodingException {
        ...
        for (File childFile : childFiles) {
            ...
            if (childFile.isDirectory()) {
                if (childPackage) {
                    myClassName.addAll(getClassNameByFile(childFile.getPath(), childPackage, packagePath));
                }
            } else {
                ...
                if (childFilePath.endsWith(".class")) {
                	...
                	int idx = childFilePath.indexOf(packagePath);
                	childFilePath = childFilePath.substring(idx>0?idx:0);
